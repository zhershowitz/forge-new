...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'forge', 'Forge', and 'BOILERPLATE'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.forge/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.forge/g),
   ...
   * Rename files.
   *
   * Rename files that had forge in their name, such as
   * css/forge.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*forge*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('forge', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-forge-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob Forge's files so that we can process them.
    glob('*', {ignore: ['node_modules/**'] }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace forge name mentions in files.
      generator.replaceFileContents();
      // Rename files that had forge in their name.
